...
<template>
  <div>
    ...
    <div v-if="isVisible === 0" class="bg-[#FEF9EF] flex flex-col items-center justify-center h-screen">
      ...
      <button @click="modePage"><img :src="playButton" alt="Play Button"
          class="w-60 h-60 mx-auto mb-[50px] mt-10 hover:scale-110"></button>
      <div class="flex gap-80 mt-10">
        <div class="flex flex-col item-center gap-2">
          ...
          <h3 class="bg-[#19C3B2] text-[#FEF9EF] text-[20px] rounded-2xl p-3">Success (0%)</h3>
        </div>
        <div class="flex flex-col item-center gap-2">
    ...
    <div v-if="isVisible === 1" class="bg-[#FEF9EF] flex flex-col items-center justify-center h-screen gap-16">
      <h1 class="text-[130px] justify-start text-[#237C9D] ">
        MODE
      </h1>
      <div class="flex flex-col gap-6">
        <button @click="playEasyMode"
          class="bg-[#19C3B2] text-[#FEF9EF] text-[40px] rounded-2xl px-8 hover:scale-110 hover:bg-[#20a396]">Easy</button>
        <button @click="playMediumMode"
          class="bg-[#FFCB77] text-[#FEF9EF] text-[40px] rounded-2xl px-8 hover:scale-110 hover:bg-[#ffb031]">
          Medium
        </button>
        <button @click="playHardMode"
          class="bg-[#FE6D73] text-[#FEF9EF] text-[40px] rounded-2xl px-8 hover:scale-110 hover:bg-[#ee464c]">
          Hard
        </button>
        <button @click="startPage"><img :src="back" alt="Back Button"
            class="w-20 h-20 mx-auto my-auto mb-1 mt-16 hover:scale-150"></button>
      </div>
    </div>
    ...
    <div v-if="isVisible === 2" class="flex flex-col justify-between bg-[#FEF9EF] h-screen">
      ...
      <div class="flex flex-col items-center justify-center flex-1 gap-8">
        <div class="flex flex-row gap-2 mb-8">
          <button v-for="word in selectedWord" :key="word" @click="selectLetter(word)"
            class="text-[40px] text-[#FEF9EF] rounded-2xl w-20 h-20 bg-[#19C3B2]">
            {{ word }}
          </button>
        </div>
        <div class="flex flex-row gap-2 mb-8">
          <div v-for="item in selectedAnswer" :key="item"
      ...
      <div class="flex justify-center items-end mb-16 gap-10">
        <button
          class="bg-[#000000] text-[#FEF9EF] text-3xl rounded-xl px-8 w-56 hover:bg-[#878787] focus:bg-black">Clear</button>
        <button
          class="bg-[#000000] text-[#FEF9EF] text-3xl rounded-xl px-8 w-56 hover:bg-[#878787] focus:bg-black">Hints ( 3
          )</button>
      </div>
    </div>
