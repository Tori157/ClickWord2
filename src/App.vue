...
const useHint = () => {
  if (hints.value > 0) {
    const restIndexes = Array
      .from({ length: correctAnswer.value.length }, (_, i) => i)
      .filter(e => !usedHintIndexes.value.includes(e));

    const randomOfRest = Math.floor(Math.random() * restIndexes.length);
    const randomIndex = restIndexes[randomOfRest];

    usedHintIndexes.value.push(randomIndex);
    selectedAnswer.value[randomIndex] = correctAnswer.value[randomIndex];
    hints.value -= 1;
    count.value++;
  }
}
    ...
    <div v-if="isVisible === 2" class="flex flex-col justify-between bg-[#FEF9EF] h-screen">
      ...
      <div class="flex flex-col items-center justify-center flex-1 gap-8">
        <div v-for="(row, rowIndex) in splitWords" :key="rowIndex" class="flex flex-row gap-2 mb-8">
          <button v-for="word in row" :key="word" @click="selectLetter(word)"
            class="text-[40px] text-[#FEF9EF] rounded-2xl w-20 h-20 bg-[#19C3B2]">
            {{ word }}
          </button>
        </div>
