...
<template>
  <div>
      ...
      <div class="flex flex-col items-center justify-center">
        <div v-for="(row, rowIndex) in splitWords" :key="rowIndex" class="flex flex-row gap-2 mb-8">
          <button v-for="item in row" :key="item.index" @click="selectLetter(item.letter, item.index)"
            ...
            :class="['text-[40px]', 'text-[#FEF9EF]', 'rounded-2xl', 'w-20', 'h-20', 'hover:bg-[#09897c]', clickedLetters[item.index] ? 'bg-[#09897c]' : 'bg-[#19C3B2]']">
            {{ item.letter.toUpperCase() }}
          </button>
        </div>
        <div class="flex flex-row gap-2 mt-10">
          <div v-for="(item, index) in selectedAnswer" :key="index"
            :class="[usedHintIndexes.includes(index) ? 'hinted-box' : !item ? 'unfilled-box' : 'filled-box']"
            class="box-base">
            {{ item.toUpperCase() }}
          </div>
        </div>
    ...
    <div v-if="isVisible === 4" class="bg-[#227C9D] h-screen flex flex-col justify-between items-center">
      <h2 class="text-white text-7xl mt-10 ">{{ `Finished Mode ${onMode.charAt(0).toUpperCase() + onMode.slice(1)}` }}</h2>
      <img :src="levelSuccess" alt="Prize" class=" w-[410px] h-[400px] items-end mt-[-10px]">
      <button @click="modePage" class=" hover:scale-150"><img :src="continueButton" alt="Continue" class=" w-[150px] h-[150px] items-end mt-[-100px] hover:scale-110"></button>
